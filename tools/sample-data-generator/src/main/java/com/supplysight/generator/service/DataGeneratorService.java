...
public class DataGeneratorService {
    ...
    private static final Map<String, double[]> INDIAN_CITIES =
            Map.of(
    ...
    private static final List<String> EVENT_SOURCES =
            List.of("GPS_DEVICE", "SCANNER", "MOBILE_APP", "API_WEBHOOK");
    ...
    public ShipmentData generateShipment(UUID tenantId) {
        ...
        return ShipmentData.builder()
                ...
                .createdAt(
                        Instant.now()
                                .minusSeconds(ThreadLocalRandom.current().nextLong(86400, 604800)))
                .build();
    }
        ...
        for (String eventType : EVENT_TYPES) {
                ...
                for (int i = 0; i < transitEvents; i++) {
                    ...
                    currentLat =
                            shipment.getOriginLat()
                                    + (shipment.getDestinationLat() - shipment.getOriginLat())
                                            * progress;
                    currentLon =
                            shipment.getOriginLon()
                                    + (shipment.getDestinationLon() - shipment.getOriginLon())
                                            * progress;
                                    ...
                                    .lon(
                                            currentLon
                                                    + ThreadLocalRandom.current()
                                                            .nextDouble(-0.1, 0.1))
                                    .hubCode(
                                            currentCity.substring(0, 3).toUpperCase()
                                                    + "-HUB-"
                                                    + String.format(
                                                            "%02d",
                                                            ThreadLocalRandom.current()
            ...
            EventData event =
                    EventData.builder()
                            ...
                            .lon(currentLon)
                            .hubCode(
                                    eventType.equals("AT_HUB")
                                                    || eventType.equals("OUT_FOR_DELIVERY")
