...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, UUID> {
    ...
    @Query(
            "SELECT rt FROM RefreshToken rt WHERE rt.tokenHash = :tokenHash AND rt.revoked = false AND rt.expiresAt > :now")
    ...
    @Modifying
    @Query(
            "UPDATE RefreshToken rt SET rt.revoked = true, rt.revokedAt = :revokedAt WHERE rt.tokenHash = :tokenHash")
