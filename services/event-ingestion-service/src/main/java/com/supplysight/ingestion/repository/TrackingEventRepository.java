...
public interface TrackingEventRepository extends JpaRepository<TrackingEventEntity, UUID> {
     ...
     */
    @Query("SELECT e FROM TrackingEventEntity e WHERE e.tenantId = :tenantId AND e.shipmentId = :shipmentId ORDER BY e.eventTime ASC")
     ...
     */
    @Query("SELECT e FROM TrackingEventEntity e WHERE e.tenantId = :tenantId ORDER BY e.ingestedAt DESC")
     ...
     */
    @Query("SELECT e FROM TrackingEventEntity e WHERE e.tenantId = :tenantId AND e.eventTime BETWEEN :startTime AND :endTime ORDER BY e.eventTime ASC")
     ...
     */
    @Query("SELECT COUNT(e) FROM TrackingEventEntity e WHERE e.tenantId = :tenantId AND e.shipmentId = :shipmentId")
    long countByTenantIdAndShipmentId(@Param("tenantId") UUID tenantId, @Param("shipmentId") UUID shipmentId);

    /**
     * Find latest event for a shipment.
     */
    @Query("SELECT e FROM TrackingEventEntity e WHERE e.tenantId = :tenantId AND e.shipmentId = :shipmentId ORDER BY e.eventTime DESC LIMIT 1")
