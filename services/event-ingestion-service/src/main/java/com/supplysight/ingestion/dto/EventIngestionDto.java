...
public final class EventIngestionDto {
    ...
    public record Location(
        Double lat,
        Double lon,
        @Size(max = 50, message = "hubCode must not exceed 50 characters")
        String hubCode
    ) {}
    ...
    public record IngestResponse(
        ...
        public static IngestResponse accepted(UUID eventId) {
            return new IngestResponse(eventId, "ACCEPTED", "Event accepted for processing", Instant.now());
        }

        public static IngestResponse duplicate(UUID eventId) {
            return new IngestResponse(eventId, "DUPLICATE", "Event already processed", Instant.now());
        }
