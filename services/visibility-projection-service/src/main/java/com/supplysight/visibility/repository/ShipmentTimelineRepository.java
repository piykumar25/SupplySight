...
public interface ShipmentTimelineRepository extends JpaRepository<ShipmentTimeline, UUID> {
     ...
     */
    @Query("SELECT t FROM ShipmentTimeline t WHERE t.tenantId = :tenantId AND t.shipmentId = :shipmentId ORDER BY t.eventTime ASC")
     ...
     */
    @Query("SELECT t FROM ShipmentTimeline t WHERE t.tenantId = :tenantId AND t.shipmentId = :shipmentId AND t.eventTime BETWEEN :startTime AND :endTime ORDER BY t.eventTime ASC")
     ...
     */
    @Query("SELECT COUNT(t) FROM ShipmentTimeline t WHERE t.tenantId = :tenantId AND t.shipmentId = :shipmentId")
     ...
     */
    @Query("SELECT t FROM ShipmentTimeline t WHERE t.tenantId = :tenantId AND t.shipmentId = :shipmentId ORDER BY t.eventTime DESC LIMIT 1")
