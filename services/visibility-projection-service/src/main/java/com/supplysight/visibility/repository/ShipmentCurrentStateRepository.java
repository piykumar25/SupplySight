...
public interface ShipmentCurrentStateRepository extends JpaRepository<ShipmentCurrentState, UUID> {
    ...
    @Query(
            "SELECT s FROM ShipmentCurrentState s WHERE s.tenantId = :tenantId AND s.shipmentId = :shipmentId")
    ...
    @Query(
            "SELECT s FROM ShipmentCurrentState s WHERE s.tenantId = :tenantId ORDER BY s.updatedAt DESC")
    Page<ShipmentCurrentState> findByTenantId(@Param("tenantId") UUID tenantId, Pageable pageable);

    /** Find shipments by status. */
    @Query(
            "SELECT s FROM ShipmentCurrentState s WHERE s.tenantId = :tenantId AND s.status = :status ORDER BY s.updatedAt DESC")
    ...
    @Query(
            "SELECT s FROM ShipmentCurrentState s WHERE s.tenantId = :tenantId AND s.status IN ('IN_TRANSIT', 'OUT_FOR_DELIVERY')")
    List<ShipmentCurrentState> findActiveShipments(@Param("tenantId") UUID tenantId);

    /** Count shipments by status. */
    @Query(
            "SELECT s.status, COUNT(s) FROM ShipmentCurrentState s WHERE s.tenantId = :tenantId GROUP BY s.status")
    List<Object[]> countByTenantIdGroupByStatus(@Param("tenantId") UUID tenantId);

    /** Check if shipment exists. */
    @Query(
            "SELECT COUNT(s) > 0 FROM ShipmentCurrentState s WHERE s.tenantId = :tenantId AND s.shipmentId = :shipmentId")
    boolean existsByTenantIdAndShipmentId(
            @Param("tenantId") UUID tenantId, @Param("shipmentId") UUID shipmentId);
}
