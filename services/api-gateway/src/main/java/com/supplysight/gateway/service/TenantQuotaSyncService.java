...
/**
 * Service for syncing tenant quotas from identity service to Redis.
 * This enables fast quota lookups at the gateway level.
 */
@Service
public class TenantQuotaSyncService {
    ...
    public Mono<Void> setQuotaLimits(UUID tenantId, int maxEventsPerSecond, int maxSseConnections) {
        ...
        return Mono.when(
                ...
                .doOnSuccess(v -> log.debug("Updated quota limits for tenant {} in Redis", tenantId));
    }
    ...
    public Mono<Void> removeQuotaLimits(UUID tenantId) {
        ...
        return Mono.when(
                redisTemplate.delete(QUOTA_KEY_PREFIX + tenantIdStr + ":max_events_per_second"),
                redisTemplate.delete(QUOTA_KEY_PREFIX + tenantIdStr + ":max_sse_connections")).then();
    }
