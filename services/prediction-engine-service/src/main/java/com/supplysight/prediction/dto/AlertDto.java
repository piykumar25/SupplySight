...
public final class AlertDto {
    ...
    public record BulkActionRequest(List<UUID> alertIds, String comment) {}

    /** Bulk action response. */
    public record BulkActionResponse(int successCount, int failureCount, List<UUID> failedIds) {}

    /** Alert count response. */
    public record AlertCount(long total, long unacknowledged, long unresolved) {}

    /** SSE event for real-time updates. */
    public record AlertEvent(String type, AlertSummary data, Instant timestamp) {}
}
