...
public interface ShipmentPredictionRepository extends JpaRepository<ShipmentPrediction, UUID> {
     ...
     */
    @Query("SELECT p FROM ShipmentPrediction p WHERE p.tenantId = :tenantId AND p.shipmentId = :shipmentId ORDER BY p.createdAt DESC LIMIT 1")
     ...
     */
    @Query("SELECT p FROM ShipmentPrediction p WHERE p.tenantId = :tenantId AND p.shipmentId = :shipmentId ORDER BY p.createdAt DESC")
     ...
     */
    @Query("SELECT p FROM ShipmentPrediction p WHERE p.tenantId = :tenantId ORDER BY p.createdAt DESC")
     ...
     */
    @Query("SELECT p FROM ShipmentPrediction p WHERE p.tenantId = :tenantId AND p.delayRisk = :delayRisk ORDER BY p.createdAt DESC")
     ...
     */
    @Query("SELECT p FROM ShipmentPrediction p WHERE p.tenantId = :tenantId AND p.anomalyDetected = true ORDER BY p.createdAt DESC")
    Page<ShipmentPrediction> findAnomaliesByTenantId(@Param("tenantId") UUID tenantId, Pageable pageable);
