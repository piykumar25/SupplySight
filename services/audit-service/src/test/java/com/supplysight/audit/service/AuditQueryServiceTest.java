...
class AuditQueryServiceTest {
    ...
    @Mock private AuditStatisticsRepository statisticsRepository;
    ...
    void getAuditLogs_shouldReturnPaginatedLogs() {
        ...
        when(auditLogRepository.findByTenantIdOrderByEventTimeDesc(
                        eq(tenantId), any(PageRequest.class)))
                .thenReturn(page);
        ...
        assertThat(result.content().get(0).getAction()).isEqualTo("LOGIN_SUCCESS");
    }
    ...
    void getAuditLogsByUser_shouldReturnUserLogs() {
        ...
        // When
        PageResponse<AuditLogDto.Response> result =
                auditQueryService.getAuditLogsByUser(tenantId, userId, 0, 20);
    ...
    void getAuditLogByCorrelationId_shouldReturnLog() {
        ...
        AuditLog log = createAuditLog();
        when(auditLogRepository.findByCorrelationId(correlationId)).thenReturn(Optional.of(log));

        // When
        Optional<AuditLogDto.Response> result =
                auditQueryService.getAuditLogByCorrelationId(correlationId);
    ...
    void getAuditSummary_shouldReturnSummaryStats() {
        ...
        when(auditLogRepository.countRecentByTenant(eq(tenantId), any(Instant.class)))
                .thenReturn(100L);
        when(auditLogRepository.countByActionForTenant(
        ...
        assertThat(summary.getActionCounts())
                .containsKeys("LOGIN_SUCCESS", "USER_CREATED", "SHIPMENT_CREATED");
    }

    @Test
    void searchAuditLogs_shouldFilterByAction() {
        // Given
        AuditLogDto.SearchRequest request =
                AuditLogDto.SearchRequest.builder()
        ...
        // When
        PageResponse<AuditLogDto.Response> result =
                auditQueryService.searchAuditLogs(tenantId, request);
