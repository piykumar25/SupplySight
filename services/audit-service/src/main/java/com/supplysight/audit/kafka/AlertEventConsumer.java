...
/** Kafka consumer for alert events. Persists alert history for tracking alert lifecycle. */
@Component
public class AlertEventConsumer {
    ...
    public AlertEventConsumer(
        ...
        this.alertsProcessedCounter =
                Counter.builder("audit.alerts.processed")
                        .description("Number of alerts processed")
                        .register(meterRegistry);
        this.alertsFailedCounter =
                Counter.builder("audit.alerts.failed")
                        .description("Number of alerts failed to process")
                        .register(meterRegistry);
    }

    @KafkaListener(
            topics = "${audit.topics.alerts}",
    ...
    public void consumeAlertEvent(String message) {
        try {
            ...
            if (alertNode.has("factors")) {
                details.put(
                        "factors", objectMapper.convertValue(alertNode.get("factors"), Map.class));
            }
            ...
            AlertHistory alertHistory =
                    AlertHistory.builder()
                            ...
                            .shipmentId(
                                    alertNode.has("shipmentId")
                                            ? UUID.fromString(alertNode.get("shipmentId").asText())
                                            : null)
                            .alertType(alertNode.get("alertType").asText())
                            .severity(
                                    alertNode.has("severity")
                                            ? alertNode.get("severity").asText()
                                            : "MEDIUM")
                            .message(
                                    alertNode.has("message")
                                            ? alertNode.get("message").asText()
                                            : null)
                            .status("OPEN")
                            .details(details)
                            .build();
            ...
            log.info(
                    "Persisted alert: {} - {} for shipment {}",
                    alertId,
                    alertHistory.getAlertType(),
                    alertHistory.getShipmentId());
