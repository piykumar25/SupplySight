...
public interface AuditStatisticsRepository extends JpaRepository<AuditStatistics, Long> {
    ...
    @Query("SELECT s FROM AuditStatistics s " +
    ...
    @Query("SELECT s.action, SUM(s.count) FROM AuditStatistics s " +
           "WHERE s.tenantId = :tenantId " +
           "AND s.statDate BETWEEN :startDate AND :endDate " +
           "GROUP BY s.action " +
    ...
    @Query("SELECT s.statDate, SUM(s.count) FROM AuditStatistics s " +
           "WHERE s.tenantId = :tenantId " +
           "AND s.statDate BETWEEN :startDate AND :endDate " +
           "GROUP BY s.statDate " +
           "ORDER BY s.statDate")
